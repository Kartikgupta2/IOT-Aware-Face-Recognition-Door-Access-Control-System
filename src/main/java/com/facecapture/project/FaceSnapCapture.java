...
//		b1.setFont(new Font("Arial",Font.BOLD,15));
		...
		JLabel l1 = new JLabel("Facial Recognition Door Access System"); // label created
		...
		b1.addActionListener(new ActionListener() { // listener on b1
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ProcessBuilder pr = new ProcessBuilder("python", "C:\\Users\\HelloAnshul\\PythonScript\\OpenWebCam.py");
					
					pr.redirectErrorStream(true); // merge stdout and stderr
					Process process = pr.start();
					//process.waitFor();
					
					// Process pr = Runtime.getRuntime()
					// 		.exec("python C:\\Users\\HelloAnshul\\PythonScript\\OpenWebCam.py");
					...
					while ((ret = in.readLine()) != null) {
						...
						if (Character.compare(ret.charAt(0), 'I') == 0) {
							//System.out.println("Calling uploadImage now");
		...
		setVisible(true);
		setTitle("Facial Recognition Door Access System");
		setResizable(false);
...
//		horizontal and vertical gap.
	...
	public void paint(Graphics g) {
		...
		g.drawString("Door Access System", 210, 145);
